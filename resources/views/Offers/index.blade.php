...
<body class="antialiased bg-gray-100" style="font-family: 'Poppins';">
    ...
    <header class="header mb-4">
        <nav class="navbar">
            ...
            <div>
                <form method="POST" action="{{ route('Offers.search') }}">
                    @csrf
    ...
    <h5 class="text-center text-2xl font-medium">
        A la recherche d'un stage ? Postuler aux offres des entreprises qui vous intéressent !
    </h5><br>

    {{-- Affichage des offres --}}
    <div class="list-offers">
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mx-6 my-3">
            @foreach ($OffersList as $item)
                <div
                    ...
                    <p
                        ...
                        <a href="{{ route('Offers.show', $item->id) }}"
                        ...
                        @if (Auth::check() && Auth::user()->roles_id === 2)
                            ...
                            $wishlist = \App\Models\Wishlist::where('users_id', Auth::id())
                                ->where('offers_id', $item->id)
                                ->first();
                            ?>
                            @if ($wishlist)
                                <form action="{{ route('wishlist.removeFromWishlist', $item->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex ">
                                        <img class="object-cover h-5 w-5 mt-2  " src="{{ asset('imgs/save.png') }}"
                                            alt="save">
                                        <p class="mx-5 mt-2"> Remove from Wishlist </p>
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('wishlist.addToWishlist', $item->id) }}" method="post">
                                    @csrf
                                    <button type="submit"class="flex ">
                                        <img class="object-cover h-5 w-5 mt-2 " src="{{ asset('imgs/!save.png') }}"
                                            alt="!save">
                                        <p class="mx-5 mt-2"> Add to Wishlist </p>
                                    </button>
                                </form>
                            @endif
                        @endif
        ...
        @if (Auth::check() && Auth::user()->roles_id === 1)
            <div class="px-4 text-xs">
                <form action="{{ route('Offers.create') }}" method="GET">
                    <button type="submit"
                        class="p-2 m-5 text-lg font-medium text-white transition duration-300 bg-[#4A7B59] rounded-sm shadow-sm hover:bg-[#649673] hover:shadow-md">
                        Créer une offre
                    </button>
                </form>
            </div>
