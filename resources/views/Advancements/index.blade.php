    ...
    <div class="list-Advancement">
        <table class="table-auto min-w-full">
            ...
            <thead>
                <tr>
                    <th
                        class="px-6 py-3 border-l border-b border-gray-300 bg-gray-50 text-center text-xs leading-4 font-medium text-gray-700 uppercase tracking-wider">
                        id</th>
                    <th
                        class="px-6 py-3 border-b border-gray-300 bg-gray-50 text-center text-xs leading-4 font-medium text-gray-700 uppercase tracking-wider">
                        steps</th>
                    ...
                    @if (Auth::check() && Auth::user()->roles_id === 1)
                        <th class="px-6 py-3 border-b border-gray-300 bg-gray-50"></th>
                        <th class="px-6 py-3 border-r border-b border-gray-300 bg-gray-50"></th>
                    @endif
                </tr>
            </thead>

            <tbody>
                @foreach ($AdvancementsList as $item)
                    {{-- @if ($item->NoAdvancement == 1) --}}
                    <tr>
                        <td
                        ...
                        <td
                            class="px-6 py-4 whitespace-no-wrap text-center border-b border-gray-300 text-sm leading-5 font-medium">
                            <a href="{{ route('Advancements.show', $item->id) }}"
                        ...
                        @if (Auth::check() && Auth::user()->roles_id === 1)
                            ...
                            <td
                                class="px-6 py-4 whitespace-no-wrap text-center border-b border-gray-300 text-sm leading-5 font-medium">
                                <a href="{{ route('Advancements.edit', $item->id) }}"
                            ...
                            <td
                                ...
                                <form action="{{ route('Advancements.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('delete')
        ...
        @if (Auth::check() && Auth::user()->roles_id === 1)
            <div class="px-4 text-xs">
                <form action="{{ route('Advancements.create') }}" method="GET">
                    <button type="submit"
